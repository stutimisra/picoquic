...
/**
 * In this function we assume that the stream is already open.
 * TODO: Add error handling around adding stream
 * */
void start_stream_cid(std::string cid, picoquic_cnx_t* connection,
		struct callback_context_t* context)
{
	printf("Starting a stream\n");

	char data[CHUNK_SIZE + 256];
	uint64_t stream_id = 0;
	context->stream_open = 1;
	context->connected = 1;
	char cid_prefix[] = "CID";
	char *char_cid = const_cast<char*>(cid.c_str()); // in addition, also append the chunk (retrieved from chunk path)


	printf("Sending %ld bytes of data on stream\n", sizeof(cid));

	ifstream myfile;
	myfile.open ("/tmp/picoquic" + cid, ios::out | ios::app | ios::binary); 	
	...
	strcpy(data, char_cid);
	strcpy(data, memblock); // this might cause a memory leak!
...
int picoquic_xia_push_client(picoquic_quic_t *client, addr_info_t myaddr, addr_info_t serveraddr, string port, LocalConfig conf) {
	...
	std::string file_name = "../TextFile1";
	...
	// putting hashfile. Note: Do we need to create table 
	cid_list_t cid_list = put_file(file_name, CHUNK_SIZE, TTL);
	...
	// Determining this to be the amount of work for each cid 
	for (auto cid: cid_list) {
		...
		if(picoquic_is_0rtt_available(connection)) {
			start_stream_cid(cid, connection, &callback_context);
		...
		if(send_length > 0) {
			...
			bytes_sent = picoquic_xia_sendmsg(myaddr.sockfd, send_buffer, (int) send_length, &serveraddr.addr, &myaddr.addr, conf);
			if(bytes_sent < 0) {
		...
		while(picoquic_get_cnx_state(connection) != picoquic_state_disconnected) {
			...
			if(bytes_recv > 0) {
				...
				if(picoquic_incoming_packet(client, buffer,
							...
							received_ecn,
							current_time)) { // picoquic_incoming_segment shows that ret = 1 is an error 
					...
					std::string token = buffer_str.substr(0, buffer_str.find(colon_delimiter));
					if (buffer_str.compare("OK") == 0) {
						printf("Chunk written successfully");
					} else {
						printf("We should retry"); // How do we retry 

						start_stream_cid(cid, connection, &callback_context); // should we do it this way or move to while loop?
			...
			while(send_length > 0) {
				...
				if(send_length > 0) {
					...
					printf("Sent %d byte packet to server: %s) from me: %s\n", bytes_sent, 
						serveraddr.dag->dag_string().c_str(), myaddr.dag->dag_string().c_str()); // should we only do this once?
				}
				pthread_mutex_unlock(&conf.lock);
			}
